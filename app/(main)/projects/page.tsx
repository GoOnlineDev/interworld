                        ...
                        {PROJECTS.map((project, i) => (
                            <div key={i} className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center group animate-reveal" style={{ animationDelay: `${(i + 1) * 200}ms` }}>
                                <div className={`${i % 2 === 1 ? 'lg:order-last' : ''} relative`}>
                                    ...
                                    {/* Icon Badge */}
                                    <div className={`absolute -bottom-6 lg:-bottom-10 ${i % 2 === 1 ? '-left-4 lg:-left-10' : '-right-4 lg:-right-10'} w-16 h-16 lg:w-24 lg:h-24 ${project.color} text-white rounded-[1.5rem] lg:rounded-[2rem] flex items-center justify-center shadow-xl shadow-black/10 transition-transform group-hover:rotate-12 duration-500`}>
                                        {iconMap[project.iconName as keyof typeof iconMap]}
                                    </div>
                                </div>
